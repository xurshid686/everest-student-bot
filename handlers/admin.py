...
async def cmd_admin(message: Message):
    ...
    await message.answer(
        ...
        "/add_recorded_lesson — Add video lesson\n"
        "/add_lesson_file — Add lesson file\n\n"
        "/show_content — View content by section\n"
...
async def do_delete(m: Message, state: FSMContext):
    ...
    except: await m.answer("Please send a valid ID number.")
...
async def do_show(m: Message, state: FSMContext):
    ...
    if s not in VALID: return await m.answer("Invalid. Try: task, homework, material, book, recorded_lesson, lesson_file")
    rows = await show_content_by_section(s)
    await state.clear()
    if not rows: return await m.answer("No content found in this section.")
...
async def cmd_stats(m: Message):
    ...
    for u in s["recent"]:
        ...
        text += f"  • {name} ({str(u['accessed_at'])[:10]})\n"
...
async def cmd_ann(m: Message, state: FSMContext):
    ...
    await m.answer("Type your announcement message:")

@router.message(Announce.waiting_message, F.text)
async def save_ann(m: Message, state: FSMContext):
    await add_announcement(m.text)
    await state.clear()
    await m.answer("✅ Announcement saved! Students will see it on /start.")
