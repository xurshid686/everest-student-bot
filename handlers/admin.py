...
from keyboards import admin_level_keyboard, admin_group_keyboard
from database import add_content, delete_content, show_content_by_section, add_announcement

router = Router()
ADMIN_ID = int(os.getenv("ADMIN_ID", "0"))

class AddContent(StatesGroup):
    choosing_level = State(); choosing_group = State()
    waiting_title = State(); waiting_content = State()
...
async def cmd_admin(message: Message):
    ...
    await message.answer("🛠 <b>Admin Commands</b>\n\n/add_task /add_homework /add_material /add_book /add_recorded_lesson /add_lesson_file\n/show_content /delete_content /announcement /cancel", parse_mode="HTML")

async def start_add(message, state, section):
    if not is_admin(message.from_user.id): return await message.answer("Not authorized.")
    await state.update_data(section=section)
    await state.set_state(AddContent.choosing_level)
    await message.answer("Choose a <b>level</b>:", reply_markup=admin_level_keyboard(), parse_mode="HTML")
...
@router.callback_query(AddContent.choosing_level, F.data.startswith("admin_level:"))
async def pick_level(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(f"Level: <b>{level}</b>\nChoose a <b>group</b>:", reply_markup=admin_group_keyboard(level), parse_mode="HTML")
...
async def pick_group(cb: CallbackQuery, state: FSMContext):
    _, level, group = cb.data.split(":", 2)
    await state.update_data(group=group)
    await state.set_state(AddContent.waiting_title)
    await cb.message.edit_text(f"Level: <b>{level}</b> | Group: <b>{group}</b>\n\nSend the <b>title</b>:", parse_mode="HTML")
...
async def get_title(m: Message, state: FSMContext):
    ...
    await m.answer("Title saved!\n\nNow send the <b>content</b> (text, photo, document, audio, video, voice).\nSend /skip for title only.", parse_mode="HTML")

@router.message(AddContent.waiting_content, F.text)
async def get_text(m: Message, state: FSMContext):
    d = await state.get_data()
    if m.text == "/skip": await add_content(d["level"], d["group"], d["section"], d["title"])
    else: await add_content(d["level"], d["group"], d["section"], d["title"], body=m.text)
...
async def get_photo(m: Message, state: FSMContext):
    d = await state.get_data()
    await add_content(d["level"], d["group"], d["section"], d["title"], body=m.caption or "", file_id=m.photo[-1].file_id, file_type="photo")
...
async def get_doc(m: Message, state: FSMContext):
    d = await state.get_data()
    await add_content(d["level"], d["group"], d["section"], d["title"], body=m.caption or m.document.file_name or "", file_id=m.document.file_id, file_type="document")
...
async def get_audio(m: Message, state: FSMContext):
    d = await state.get_data()
    await add_content(d["level"], d["group"], d["section"], d["title"], body=m.caption or "", file_id=m.audio.file_id, file_type="audio")
...
async def get_video(m: Message, state: FSMContext):
    d = await state.get_data()
    await add_content(d["level"], d["group"], d["section"], d["title"], body=m.caption or "", file_id=m.video.file_id, file_type="video")
...
async def get_voice(m: Message, state: FSMContext):
    d = await state.get_data()
    await add_content(d["level"], d["group"], d["section"], d["title"], file_id=m.voice.file_id, file_type="voice")
...
async def cmd_delete(m: Message, state: FSMContext):
    ...
    await m.answer("Send the <b>content ID</b> to delete. Use /show_content to find IDs.", parse_mode="HTML")
...
async def cmd_show(m: Message, state: FSMContext):
    ...
    await m.answer("Which section?\n<code>task | homework | material | book | recorded_lesson | lesson_file</code>", parse_mode="HTML")
...
async def do_show(m: Message, state: FSMContext):
    ...
    for r in rows:
        text += f"ID: <code>{r['id']}</code> | {r['level']} / {r['group_name']}\nTitle: {r['title']}\n{'File: '+r['file_type'] if r['file_id'] else 'Text only'}\n\n"
    await m.answer(text[:4000], parse_mode="HTML")
