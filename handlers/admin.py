...
async def start_add(message, state, section):
    ...
    await message.answer("Choose a <b>group</b>:", reply_markup=admin_group_keyboard(), parse_mode="HTML")
...
async def get_title(m: Message, state: FSMContext):
    ...
    await m.answer("Title saved!\n\nSend <b>content</b>: text, photo, document, audio, video, voice.\nOr /skip for title only.", parse_mode="HTML")
