...
def sections_keyboard(group):
    ...
    buttons.append([InlineKeyboardButton(text="Back to Groups", callback_data="back:groups")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)

def back_to_sections_keyboard(group):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Back to Sections", callback_data=f"back:sections:{group}")]
    ])
