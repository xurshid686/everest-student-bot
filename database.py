...
async def get_announcements():
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute("SELECT * FROM announcements ORDER BY created_at DESC LIMIT 5")
        return [dict(r) for r in await cursor.fetchall()]
...
async def get_stats():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # Most active group
        c2 = await db.execute("""
            SELECT group_name, COUNT(*) as cnt FROM stats
        ...
        c3 = await db.execute("""
            SELECT section, COUNT(*) as cnt FROM stats
        ...
        # Recent users
        c5 = await db.execute("""
            SELECT DISTINCT user_id, username, full_name, MAX(accessed_at) as last
            FROM stats GROUP BY user_id ORDER BY last DESC LIMIT 10
        """)
